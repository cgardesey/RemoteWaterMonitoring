...
import com.remote.water.monitoring.util.notimportant.DemoBase;
...
//        l.setYOffset(11f);
    ...
    private void setData(int count, float range) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(getApplicationContext())).executeTransaction(realm -> {
            RealmResults<RealmMeasurement> results = realm.where(RealmMeasurement.class)
                    .findAll()
                    .sort("id", Sort.ASCENDING);
            OrderedRealmCollectionSnapshot<RealmMeasurement> resultsSnapshot = results.createSnapshot();// <-- snapshot

            for (int i = 0; i < resultsSnapshot.size(); i++) {
//                Log.d("asdffds", String.valueOf(resultsSnapshot.get(i).getId()));
                values1.add(new Entry(i, resultsSnapshot.get(i).getWaterlevel()));
            }
        });
        ...
        if (chart.getData() != null &&
            ...
            set1 = new LineDataSet(values1, "Water Level");
